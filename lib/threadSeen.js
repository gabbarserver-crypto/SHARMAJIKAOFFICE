...
function identityKey(identity) {
  ...
  return `${identity.type || "unknown"}:${identity.id || "unknown"}`;
}
...
export function saveSeenMap(identity, map) {
  try {
    ...
    window.dispatchEvent(new CustomEvent("sjo:thread-seen", { detail: { identityKey: identityKey(identity) } }));
  } catch {
...
// Marks one thread as seen and notifies the rest of the app immediately.
// This is intentionally shared by every chat surface (full inbox, popup,
// application chat modal, etc.) so a server-side read receipt and the badge
// state cannot drift apart.
export function markThreadSeen(identity, threadId, seenAt = new Date().toISOString()) {
  if (!threadId) return {};
  const map = loadSeenMap(identity);
  const next = { ...map, [threadId]: seenAt };
