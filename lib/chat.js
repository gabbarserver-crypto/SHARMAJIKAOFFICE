...
import { loadSeenMap, isThreadSeen } from "./threadSeen";
...
// unread count is based on the current identity's seen/read timestamp.
export async function countOpenThreads(identity = null) {
  ...
  const seenMap = loadSeenMap(identity);
  const latestByThread = {};
  for (const m of messages || []) {
    if (!latestByThread[m.thread_id]) latestByThread[m.thread_id] = m;
  }

  // Badge = genuinely unread application threads, not merely threads whose
  // latest message is from the dealer. Once the user opens a thread (or
  // ChatPanel records its read receipt), threadSeen.js updates this map and
  // the badge drops immediately.
  return Object.values(latestByThread).filter(
    (m) => m.sender_type !== "staff" && !isThreadSeen(seenMap, m.thread_id, m.created_at)
  ).length;
}
...
export async function countDealerUnread(dealerId, identity = null) {
  const { data: threads, error: threadsError } = await supabase
    .from("chat_threads")
    .select("id")
